<?php
/**
 * Modified Sendy Subscribe Handler with hCaptcha Integration
 * 
 * This is a customized version of Sendy's subscribe.php that adds:
 * - hCaptcha verification for all POST requests
 * - Support for external form handlers
 * - Enhanced bot protection
 * - IP address forwarding from proxy handlers
 * 
 * Based on original Sendy subscribe.php with security enhancements.
 * 
 * @author Brett Whiteside (modifications)
 * @copyright 2025 Brett Whiteside (modifications only)
 * @license MIT (for modifications)
 * @version 1.0.0
 * @modified August 2025
 * 
 * Original Sendy software copyright Sendy.co
 * This file contains modifications to integrate with hcaptcha-handler.php
 * See README.md for configuration requirements.
 */
	...
	//--------------------------------------------------------------//
	    global $mysqli, $dbHost, $dbUser, $dbPass, $dbName, $dbPort;
...
<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Skip hCaptcha check if valid API key is provided
        $api_key = isset($_POST['api_key']) ? mysqli_real_escape_string($mysqli, trim($_POST['api_key'])) : '';
        
        // Get the user's API key for comparison
        if($api_key != '') {
            $list_id_check = isset($_POST['list']) ? strip_tags(decrypt_int(mysqli_real_escape_string($mysqli, $_POST['list']))) : 0;
            if($list_id_check > 0) {
                $q_api = 'SELECT login.api_key FROM lists, login WHERE lists.id = '.$list_id_check.' AND login.id = lists.userID';
                $r_api = mysqli_query($mysqli, $q_api);
                if ($r_api && mysqli_num_rows($r_api) > 0) {
                    while($row = mysqli_fetch_array($r_api)) {
                        $user_api_key_check = $row['api_key'];
                    }
                    // If API key matches, skip hCaptcha
                    if($api_key == $user_api_key_check) {
                        goto skip_hcaptcha;
                    }
                }
            }
        ...
        // Continue with hCaptcha check for non-API requests
        $hCaptchaResponse = trim((string)($_POST['h-captcha-response'] ?? ''));
        $ipaddress = ipaddress(); // Get IP early for verification
        if ($hCaptchaResponse === '') {
            if (isset($_POST['boolean']) && $_POST['boolean'] == 'true') {
                echo 'Please complete the hCaptcha.';
                exit;
            } else {
                $feedback = _('Please complete the hCaptcha.');
                goto output;
            }
        } else {
            $ch = curl_init();
            ...
            if (curl_error($ch)) {
                curl_close($ch);
                if (isset($_POST['boolean']) && $_POST['boolean'] == 'true') {
                    echo 'Network error during hCaptcha verification.';
                    exit;
                } else {
                    $feedback = _('Network error during hCaptcha verification.');
                    goto output;
                }
            }
            curl_close($ch);
            ...
            if (!isset($captcha['success']) || !$captcha['success']) {
                if (isset($_POST['boolean']) && $_POST['boolean'] == 'true') {
                    ...
                    $feedback = _('hCaptcha verification failed.');
                    goto output;
                }
            }
        }
    ...
    // How are bots still getting through
    $debug_info = "SENDY DEBUG - Time: " . date('Y-m-d H:i:s') . " | IP: " . (function_exists('ipaddress') ? ipaddress() : $_SERVER['REMOTE_ADDR']) . " | Email: " . (isset($_POST['email']) ? $_POST['email'] : 'not set') . " | API Key: " . (isset($_POST['api_key']) ? 'YES' : 'none') . " | User Agent: " . ($_SERVER['HTTP_USER_AGENT'] ?? 'none');
    error_log($debug_info);
    // Debug which path we're taking
    error_log("SENDY PATH - subform: " . (isset($_POST['subform']) && $_POST['subform']=='yes' ? 'YES' : 'NO'));
	...
	{		
		...
		$list_id = strip_tags(decrypt_int(mysqli_real_escape_string($mysqli, $_POST['list'])));
		$return_boolean = isset($_POST['boolean']) ? strip_tags(mysqli_real_escape_string($mysqli, $_POST['boolean'])) : '';
		$hp = isset($_POST['hp']) ? strip_tags(mysqli_real_escape_string($mysqli, $_POST['hp'])) : '';
		$ipaddress = isset($_POST['ipaddress']) ? strip_tags(mysqli_real_escape_string($mysqli, $_POST['ipaddress'])) : '';
		$country = isset($_POST['country']) ? strip_tags(mysqli_real_escape_string($mysqli, $_POST['country'])) : '';
		$referrer = isset($_POST['referrer']) ? strip_tags(mysqli_real_escape_string($mysqli, $_POST['referrer'])) : '';
		$notes = isset($_POST['notes']) ? strip_tags(mysqli_real_escape_string($mysqli, $_POST['notes'])) : '';
	...
	//get data from apps
	$q = 'SELECT from_name, from_email, reply_to, smtp_host, smtp_port, smtp_ssl, smtp_username, smtp_password, allocated_quota, custom_domain, custom_domain_protocol, custom_domain_enabled FROM apps WHERE id = '.$app;
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
		while($row = mysqli_fetch_array($r))
	...
	{
		//IP
		// Check if IP was passed in POST (from our handler)
        if(isset($_POST['ipaddress']) && filter_var($_POST['ipaddress'], FILTER_VALIDATE_IP)) {
	...
	if($feedback!=_('Some fields are missing.') && $feedback!=_('Email address is invalid.') && $feedback!=_('Email address is bounced.') && $feedback!=_('Consent not given.') && $feedback!=_('Invalid secret key.') && $feedback!=_('API key not passed') && $feedback!=_('Invalid API key') && $feedback!=_('hCaptcha verification failed.') && $feedback!=_('Network error during hCaptcha verification.') && $feedback!=_('Please complete the hCaptcha.'))
	{		
		...
		if ($r && mysqli_num_rows($r) > 0)
		{
			...
			if($opt_in) 
            {
                ...
                $q = 'UPDATE subscribers SET unsubscribed = 0, last_campaign = NULL, timestamp = '.$time.', confirmed = '.$confirmed.', '.$name_line.' custom_fields = "'.substr($cf_value, 0, -3).'" '.$gdpr1.', notes = "'.$notes.'", ip = "'.$ipaddress.'", country = "'.$country.'", referrer = "'.$referrer.'", added_via = '.$added_via.' WHERE email = "'.$email.'" AND list = '.$list_id;
            }
			else
            {
                $name_line = !isset($_POST['name']) ? '' : ', name = "'.$name.'"';
                $q = 'UPDATE subscribers SET unsubscribed = 0, last_campaign = NULL, timestamp = '.$time.', confirmed = 1 '.$name_line.', custom_fields = "'.substr($cf_value, 0, -3).'" '.$gdpr1.', notes = "'.$notes.'", ip = "'.$ipaddress.'", country = "'.$country.'", referrer = "'.$referrer.'", added_via = '.$added_via.' WHERE email = "'.$email.'" AND list = '.$list_id;
            }
			$r = mysqli_query($mysqli, $q);
			if ($r)
			{
...
output:
	if($subscribed_url != '' && !$already_subscribed && $feedback!=_('Some fields are missing.') && $feedback!=_('Email address is invalid.') && $feedback!=_('Email address is bounced.') && $feedback!=_('Consent not given.') && $feedback!=_('Invalid secret key.') && $feedback!=_('API key not passed') && $feedback!=_('Invalid API key') && $feedback!=_('hCaptcha verification failed.') && $feedback!=_('Network error during hCaptcha verification.') && $feedback!=_('Please complete the hCaptcha.'))
	{
